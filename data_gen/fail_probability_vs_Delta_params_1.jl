...
# Initial state #
#################
final_fidelity = zeros(N_tot_runs,length(Δ_array))
pbar = ProgressBar(total=length(final_fidelity))
for (k,Δ) in enumerate(Δ_array)
    for m=1:N_tot_runs
        ...
        final_fidelity[m,k] = norm(ψ_t'*ψ_0)^2    
    end
    update(pbar)
end
